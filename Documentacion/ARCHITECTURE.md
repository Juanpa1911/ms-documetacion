...
El proyecto implementa **Test-Driven Development (TDD)** con múltiples niveles de pruebas:

#### Niveles de Testing
- **Unitarios**: Lógica de servicios aislada (75 tests)
- **Integración**: Flujos con mocks de Redis/HTTP
- **E2E**: Generación completa de documentos (PDF, DOCX, ODT)
- **Rendimiento**: k6 tests (smoke, load, spike)

#### Cobertura Actual
- **Tests unitarios**: 75 tests pasando (100%)
- **Coverage**: >70% del código
- **Frameworks**: unittest + coverage

#### Tests por Componente
| Componente | Tests | Descripción |
|------------|-------|-------------|
| **Retry Pattern** | 10 | Decorator con backoff exponencial |
| **Validators** | 12 | Validación de alumnos y contexto |
| **Repositories** | 8 | Cache y servicios externos |
| **Middleware** | 5 | Error handling y logging |
| **App** | 16 | Endpoints y configuración |
| **Integration** | 24 | Flujos completos end-to-end |
...
El microservicio cumple con los [12 factores](https://12factor.net/) para aplicaciones cloud-native:

| Factor | Implementación |
|--------|---------------|
| **I. Codebase** | ✅ Un repositorio Git, múltiples despliegues (dev, prod) |
| **II. Dependencies** | ✅ `pyproject.toml` + `uv.lock` con dependencias explícitas |
| **III. Config** | ✅ Variables de entorno (`.env`, no hardcoded) |
| **IV. Backing Services** | ✅ Redis y microservicios como recursos adjuntos |
| **V. Build, Release, Run** | ✅ Dockerfile + Docker Compose, separación clara |
| **VI. Processes** | ✅ Stateless, toda la data en Redis (sin archivos locales) |
| **VII. Port Binding** | ✅ Exporta servicio vía puerto 5000 (Granian) |
| **VIII. Concurrency** | ✅ Escalado horizontal con réplicas Docker |
| **IX. Disposability** | ✅ Inicio rápido (<40s), shutdown graceful |
| **X. Dev/Prod Parity** | ✅ Docker Compose idéntico en ambos entornos |
| **XI. Logs** | ✅ Logs a stdout/stderr (capturados por Docker) |
| **XII. Admin Processes** | ✅ Scripts independientes (migrations futuras) |

### Principios DRY, KISS, SOLID

Ver detalles completos en [PATRONES_MICROSERVICIOS.md](../PATRONES_MICROSERVICIOS.md).

**Resumen:**
- ✅ **DRY**: Decorator `@retry` reutilizable, cliente Redis centralizado
- ✅ **KISS**: Implementaciones simples, configuración declarativa
